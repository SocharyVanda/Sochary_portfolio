...
/**
 * Vanilla-JS port of the React Bits FloatingLines component.
 * Mounts a full-bleed WebGL shader canvas into `container` and returns a
 * destroy() function that tears everything down (mirrors the React useEffect cleanup).
 */
export function initFloatingLines(container, options = {}) {
  ...
  container.classList.add("floating-lines-container");
  ...
  const ro = typeof ResizeObserver !== "undefined"
    ? new ResizeObserver(function () { if (active) setSize(); })
    : null;
  ...
  function handlePointerMove(event) {
    ...
    if (parallax) {
      ...
      targetParallax.set(((x - centerX) / rect.width) * parallaxStrength, (-(y - centerY) / rect.height) * parallaxStrength);
    }
  }
  ...
  function renderLoop() {
    ...
    if (!reduceMotion) uniforms.iTime.value = clock.getElapsedTime();
